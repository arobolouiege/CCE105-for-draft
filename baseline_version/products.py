...
class ProductsPage(BasePage):
    ...
    def build(self):
        ...
        search_frame.grid(row=5, column=0, columnspan=4, pady=(8, 2), padx=8, sticky="w")
        tk.Label(search_frame, text="Product Search", font=FONT_SMALL,
                 bg=WHITE, fg=TEXT_GRAY).pack(side="left")
        self._v_search = tk.StringVar()
        self._v_search.trace_add("write", self._run_search)
        tk.Entry(search_frame, textvariable=self._v_search, width=30,
                 relief="solid", bd=1).pack(side="left", padx=6)
        self._lbl_search_time = tk.Label(
            search_frame, text="Baseline linear search: 0.00 µs",
            font=FONT_SMALL, bg=WHITE, fg=SIDEBAR)
        self._lbl_search_time.pack(side="left", padx=8)
        ...
        btn_frame.grid(row=6, column=0, columnspan=4, pady=8, padx=6, sticky="w")
        ...
        tk.Label(form, text="💡 Click a row to edit it.", font=FONT_SMALL,
                 bg=WHITE, fg=TEXT_GRAY).grid(row=7, column=0, columnspan=4,
    ...
    def _run_search(self, *_):
        ...
        if query:
            ...
            for r in self._all_rows:
                if r["product_name"].lower().startswith(query):
                    rows.append(r)
        else:
            rows = self._all_rows
        elapsed = (time.perf_counter() - start) * 1_000_000

        self._lbl_search_time.config(
            text=f"Baseline linear search: {elapsed:.2f} µs")
