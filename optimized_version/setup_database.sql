...
SELECT c.category_id, seed.product_name, seed.unit, seed.price, seed.reorder_level
FROM (
    SELECT 'Canned Goods' AS category_name, 'San Marino Tuna Paella 180g' AS product_name, 'can' AS unit, 47.00 AS price, 8 AS reorder_level UNION ALL
    SELECT 'Beverages', 'Kopiko Lucky Day 180ml', 'bottle', 28.00, 10 UNION ALL
    SELECT 'Snacks', 'Oishi Sponge Crunch Chocolate 60g', 'pack', 18.00, 15 UNION ALL
    SELECT 'Noodles', 'Lucky Me Chicken Sotanghon Cup', 'cup', 39.00, 10 UNION ALL
    SELECT 'Household', 'Joy Antibac Dishwashing Sachet', 'sachet', 8.00, 25 UNION ALL
    SELECT 'Beverages', 'Milo Champorado Sachet 35g', 'sachet', 14.00, 20 UNION ALL
    SELECT 'Snacks', 'Rebisco Choco Sandwich 32g', 'pack', 9.00, 25
) seed
JOIN category c ON c.category_name = seed.category_name
WHERE NOT EXISTS (
    SELECT 1 FROM product p WHERE p.product_name = seed.product_name
