...
class DashboardPage(BasePage):
    ...
    def build(self):
        ...
        tk.Label(self, text="Top Selling Products",
                 font=FONT_HEADER, bg=BG, fg=TEXT_DARK).pack(
            anchor="w", padx=14, pady=(10, 2))
        self._lbl_rank_info = tk.Label(self, text="Sorted using Merge Sort",
                                       font=FONT_SMALL, bg=BG, fg=ACCENT)
        self._lbl_rank_info.pack(anchor="w", padx=14)
        rank_cols = ("Rank", "Product", "Units Sold", "Total Revenue")
        rank_widths = (60, 220, 90, 120)
        self._rank_tree = self.make_table(self, rank_cols, rank_widths, height=6)
    ...
    def _refresh_top_sellers(self):
        ...
        """)
        # DashboardPage = Merge Sort for the operational top-selling leaderboard.
        ranked = merge_sort(rows, key=lambda r: float(r["revenue"] or 0), reverse=True)
        ...
        self._lbl_rank_info.config(text="Sorted using Merge Sort")
